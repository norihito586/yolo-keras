...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            #車のラベルがあったなら画像を保存する
            if 'car' in label:
                carImage = image.crop((left, top, right, bottom))
                carImage = np.asarray(carImage)
                print(carImage.shape)
                files = os.listdir("carImage")
                count_num = len(files)
                cv2.imwrite("carImage/carImage" + str(count_num) + ".png", carImage)
            else:
                pass
    ...
    #import pdb; pdb.set_trace() #デバッグはこれを挿入！！
def detect_video(yolo, video_path=1, output_path=""):
    import cv2
